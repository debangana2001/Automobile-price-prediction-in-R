---
title: "Automobile_Price_Prediction"
...
```{r}
# Let's load the data set here:-
data<-read.csv("C:\\Users\\DIBYAKANTI\\Downloads\\CarPrice.csv", head=TRUE)
head(data)
```
```{r}
...
plot_correlation(
 ...
 axis.text.x = element_text(hjust = 1, angle = 45,size = 
15)))
...
pl2 <- ggplot(data = data, aes(x = price)) +
 ...
 sd = sd(data$price))) +
 ggtitle("Histogram with theoretical normal dist. curve") +
 theme_bw() +
 theme(legend.position = "none",
 axis.text = element_text(size = 15),
 axis.title = element_text(size=15)) 
...
pl2 <- ggplot(data = data, aes(x = TRF_price)) +
 ...
 ggtitle("Transformed Histogram with theoretical normal dist. curve") +
 theme_bw() +
 theme(legend.position = "none",
...
qqline(data$price)
qqnorm(data$TRF_price, pch = 19, col = "gray50", main = "Normal Q-Q Plot 
TRF_Price")
...
for (i in nombre_col) {
 ...
 data.frame(
 Variable = i,
 Intercept = round(sumario$coefficients[1], 2), beta = 
round(sumario$coefficients[2], 4),
 t_value = round(sumario$coefficients[6], 2), p_value = 
sumario$coefficients[8],
...
desechadas <- c("symboling", "stroke", "carheight", "compressionratio", 
"peakrpm",
...
desechadas <- c("symboling", "stroke", "carheight", "compressionratio", 
"peakrpm", "fueltype", "doornumber","citympg", "highwaympg", "price", 
...
desechadas <- c("symboling", "stroke", "carheight", "compressionratio", 
"peakrpm", "fueltype",
 "doornumber", "citympg", "highwaympg", "price", "enginesize", 
"boreratio", "enginetype" )
# Data frame with significant variables for the model
CarPrice_predictoras <- data[ , !(names(data) %in% desechadas)]
# New backward model with less variables
modelo <- lm(TRF_price ~., data = CarPrice_predictoras)
...
ggplot(data = modelo_backward3, aes(x = modelo_backward3$residuals)) +
 ...
 ggtitle("Residuals histogram with theoretical normal dist. curve") +
 theme_bw() +
